...
const proposeSkillSchema = Type.Object({
  ...
  description: Type.String({ description: "一句话说明这个技能的作用，可为空", maxLength: SKILL_DESCRIPTION_MAX }),
...
export function createProposeSkillTool(): AgentTool<typeof proposeSkillSchema, SkillProposalDetails> {
  return {
    ...
    label: "提议沉淀技能",
    description:
      "当你在对话中发现卖家反复提出同一类要求（例如多次要求强调包邮、固定的话术风格或输出格式）时，调用本工具提议把该模式沉淀为可复用技能。只提议、不创建，技能必须等卖家在卡片上确认后才创建。",
        ...
        content: [
          {
            type: "text",
            text:
              `建议把「${params.name}」沉淀为可复用技能：${params.description === "" ? "以后自动应用这条要求" : params.description}。请卖家在卡片上确认是否创建。`,
          },
        ],
        details: { name: params.name, description: params.description, prompt: params.prompt },
      };
    },
  };
}
