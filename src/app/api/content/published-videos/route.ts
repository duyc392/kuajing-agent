// 用途：内容运营-已发布视频接口：GET 读取指定商品的已发布视频及投放数据（先校验店铺与商品归属，再读数据源）。
import { z } from "zod";
import { toErrorResponse } from "@/lib/api-error";
import { readShopId } from "@/lib/request-shop";
import { loadPublishedVideos } from "@/services/content-ops.service";
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json(await loadPublishedVideos(readShopId(request), productId));
  } catch (error) {
