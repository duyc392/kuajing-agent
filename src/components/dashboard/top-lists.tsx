  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        <p className="text-sm font-medium text-gray-900">内容表现（Top 视频）</p>
        ...
        {items.map((item, index) => (
          <div key={index} className="grid gap-1">
            ...
            <div className="h-1.5 w-full rounded bg-gray-100">
              <div className="h-1.5 rounded bg-blue-500" style={{ width: `${Math.max((item.plays / maxPlays) * 100, 2)}%` }} />
            </div>
          </div>
        ))}
      </div>
...
export function CategoryPerformance({ items }: { items: DashboardCategoryItem[] }) {
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        <p className="text-sm font-medium text-gray-900">类目表现（按 GMV）</p>
        ...
        {items.map((item, index) => (
          <div key={index} className="grid gap-1">
            <div className="flex items-baseline justify-between gap-2">
              ...
              <p className="shrink-0 text-xs text-gray-500">{formatPercent(item.gmvSharePercent)}</p>
            </div>
            <div className="h-1.5 w-full rounded bg-gray-100">
