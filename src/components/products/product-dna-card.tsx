...
const DNA_FIELDS = [
  { key: "targetPersona", label: "核心用户画像", enLabel: "Persona", icon: "🎯", placeholder: "如：18-34 岁女性，通勤与学生党，追求精致但预算有限" },
  { key: "useScenarios", label: "核心使用场景", enLabel: "Scene", icon: "🏠", placeholder: "如：通勤补妆、宿舍化妆台、旅行收纳" },
  { key: "coreSellingPoints", label: "核心功能卖点", enLabel: "USP", icon: "⭐", placeholder: "如：三档补光还原自然色；折叠后仅 1cm 随身携带" },
  { key: "visualHooks", label: "视觉冲击效果", enLabel: "Visual", icon: "👁️", placeholder: "如：开灯瞬间脸部亮度对比；粉饼盒大小折叠特写" },
  { key: "recommendedFormats", label: "推荐视频呈现方式", enLabel: "Format", icon: "🎭", placeholder: "如：对比（开灯前后）、教程（通勤快速妆容）、种草" },
  { key: "competitorDifferences", label: "竞品差异与替代痛点", enLabel: "Difference", icon: "⚖️", placeholder: "如：同价位多为单档补光，本品三档调光且 USB 充电复用" },
] as const;
  ...
  return (
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {DNA_FIELDS.map((field) => (
        <div key={field.key} className="rounded-xl border border-blue-100 bg-blue-50/40 p-3.5">
          <p className="flex items-center gap-1.5 text-sm font-semibold text-gray-900">
            <span>{field.icon}</span>
            {field.label}
            <span className="text-xs font-normal text-gray-400">({field.enLabel})</span>
          </p>
          <p className="mt-1.5 text-sm whitespace-pre-wrap text-gray-700">{dna[field.key]}</p>
        </div>
      ))}
    </div>
  );
...
export default function ProductDnaCard({ productId, shopId }: DnaCardProps) {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-r from-blue-500 via-sky-400 to-purple-500 p-[1.5px]">
      <div className="rounded-[calc(1rem-1.5px)] bg-white p-5">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-gray-900">
            ✨ 六维商品 DNA
            <span className="ml-2 text-sm font-normal text-gray-500">{dna ? `（已提取 6 维度 · 支持随时微调）` : "（未建立）"}</span>
          </h2>
          {editing ? (
            <div className="flex gap-2">
              <button onClick={handleSave} disabled={saving} className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700 disabled:opacity-50">
                {saving ? "保存中…" : dna ? "保存覆盖" : "建立 DNA"}
              </button>
              <button onClick={exitEdit} className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100">
                取消
              </button>
            </div>
          ) : (
            <button onClick={startEdit} className="rounded-lg border border-blue-300 px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-50">
              {dna ? "编辑" : "＋ 建立 DNA"}
            </button>
          )}
        </div>
        {editing ? (
          <div className="mt-4 grid gap-3">
            {DNA_FIELDS.map((field) => (
              <label key={field.key} className="grid gap-1 text-xs text-gray-600">
                {field.label}
                <textarea
            ...
            <p className="text-xs text-gray-400">保存会覆盖当前 DNA，不会保留历史版本。</p>
          </div>
        ) : dna ? (
          <DnaReadView dna={dna} />
        ) : (
        ...
        {dna && !editing && <p className="mt-3 text-xs text-gray-400">上次更新：{dna.updatedAt.slice(0, 10)}</p>}
      </div>
    </div>
  );
}
