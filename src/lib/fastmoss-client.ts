...
// FastMoss 配置来自本机 data/config.json（Key 只存服务端，不进响应）；未配置 Key 时按功能名给中文引导。
// 看板（services）与选品（agent）共用，避免同一读取逻辑写两遍。
export async function readFastmossSettings(feature: string): Promise<FastmossSettings> {
  const keys = await readKeysConfig();
  if (!keys.fastmoss.apiKey) throw new ValidationError(`尚未配置 FastMoss API Key，请先在设置页完成配置后再使用${feature}`);
